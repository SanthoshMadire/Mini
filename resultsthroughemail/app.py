...
def getcsv():
    if request.method == 'POST':
       subject='Dear {name}, your Semester marks '
       body= '\n CVR COLLEGE OF ENGINEERING \n\n {sub1} = {s1} \n {sub2} = {s2} \n {sub3} = {s3} \n {sub4} = {s4} \n {sub5} = {s5} \n Your Obtained Marks are :{total} \n Status : {status} \n Regards, \n cvr.ac.in \n'
       ...
       with smtplib.SMTP_SSL("smtp.gmail.com", 465, context=context) as server:
          ...
          with open(f.filename) as file:
              ...
              for ID,NAME,BRANCH,SECTION,SUB1,S1,SUB2,S2,SUB3,S3,SUB4,S4,SUB5,S5,TOTAL,STATUS,EMAIL in reader:
                   server.sendmail(from_address,EMAIL,message.format(name=NAME,sub1=SUB1,s1=S1,sub2=SUB2,s2=S2,sub3=SUB3,s3=S3,sub4=SUB4,s4=S4,sub5=SUB5,s5=S5,total=TOTAL,status=STATUS),)
       f=request.files['datacsv']
       results=[]
       with open(f.filename) as file:
